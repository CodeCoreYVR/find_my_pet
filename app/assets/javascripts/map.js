  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function (position) {
      ...
      if (!$('#show-map').html() && !$('#sightings-map').html()) {
        ...
        var slugArr = /(\d+)$/.exec(document.URL);
        var myPet = window.globals.allpets.filter(function (data) {
          return data.id == slugArr[0];
        });
...
var getLostPets = function getLostPets() {
...
var getCloseLostPets = function getCloseLostPets() {
  // Get all the lost pets and draw markers on the map
  $.get('/api/v1/pets', { user_lat: globals.location.lat, user_lng: globals.location.lng }, function (data) {
...
var getPetsInView = function getPetsInView(bounds) {
  // Get all the lost pets and draw markers on the map
  // var bounds = map.getBounds();
  var ne = bounds.getNorthEast();
  var sw = bounds.getSouthWest();
  $.get('/api/v1/pets', { top: ne.lat,
    bottom: sw.lat,
    left: sw.lng,
    right: ne.lng }, function (data) {
    window.globals.pets = data;
    for (var i = 0; i < data.length; i++) {
...
var getSightingsOfaPet = function getSightingsOfaPet(pet_id) {
  // Get all the lost pets and draw markers on the map
  $.ajax({
...
function getPetsByType(pets_data, type, map) {
  var pets = pets_data;
  return pets.filter(function (data) {
    return pets.pet_type == type;
  });
}
